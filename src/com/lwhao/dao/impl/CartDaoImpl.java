...
public class CartDaoImpl extends BaseDao implements CartDao {
    ...
    public int addCartItem(CartItem cartItem, int userId) {
        ...
        String checkSql = "SELECT quantity FROM t_cart WHERE user_id = ? AND product_id = ?";
    ...
    public int updateCartItem(CartItem cartItem, int userId) {
        String sql = "UPDATE t_cart SET quantity = ? WHERE user_id = ? AND product_id = ?";
        return update(sql, cartItem.getCount(), userId, cartItem.getId());
    }

    @Override
    public List<CartItem> getCartItems(int userId) {
        String sql = "SELECT product_id AS id, quantity AS count FROM t_cart WHERE user_id = ?";
